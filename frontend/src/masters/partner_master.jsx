...
const PartnerMaster = () => {
    ...
    const fetchPartnerData = () => {
        axios.get('http://localhost:3000/api/partner') // Replace with your backend endpoint
            .then(response => {
    ...
    const handleFormSubmit = async (formData) => {
        ...
        try {
            const response = await axios.post('http://localhost:3000/api/partner/partner-form', formData);
    ...
    const handleDelete = async (partner_id) => {
        try {
            const response = await axios.post('http://localhost:3000/api/partner/inactivate', { partner_id });
            if (response.data.success) {
    ...
    const handleActivate = async (partner_id) => {
        try {
            const response = await axios.post('http://localhost:3000/api/partner/activate', { partner_id });
            if (response.data.success) {
                fetchPartnerData(); // Refresh data after successful deletion
            } else {
    ...
    //     axios.get(`http://localhost:3000/api/partner?search=${searchQuery}&status=${statusFilter}`)
    //         .then(response => {
    //             setPartners(response.data);
    //         })
    //         .catch(error => {
    ...
    return (
        <div>
            ...
            <div className="overflow-x-auto shadow-md absolute right-0 w-5/6">
                <p className='bg-gray-200 border-gray-200 p-3 m-0 dark:bg-gray-800 relative self-right text-xl font-semibold whitespace-nowrap dark:text-gray-400'>Partner Master</p>
                <input
                    type="text"
                    id="search"
                    value={searchQuery}
