...
const CategoryMaster = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: row => (
                ...
                <button
                    title='Delete'
                    onClick={() => handleDelete(row.cat_id)}
                ...
                <button
                    title='Activate'
                    onClick={() => handleActivate(row.cat_id)}
                    className="bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-700"
                >
                    <img 
    ...
    const filteredItems = useMemo(() => {
        return categories.filter(item => {
            const matchesFilter = item.category.toLowerCase().includes(filterText.toLowerCase());
            const matchesStatus = statusFilter === 'all' || 
                                (statusFilter === 'active' && item.status) || 
                                (statusFilter === 'inactive' && !item.status);
            return matchesFilter && matchesStatus;
        });
    }, [categories, filterText, statusFilter]);
        ...
        <div>
            ...
            <div className="overflow-x-auto shadow-md absolute right-0 w-5/6 px-6 py-3">
                <div className="flex justify-between items-center mb-4">
                    <p className='text-2xl font-bold text-gray-700'>Category Master</p>
