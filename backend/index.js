...
app.get('/api/dropdown-values', async (req, res) => {
    try {
        ...
        const usernames = await pool.query("SELECT * FROM User_Master WHERE role IN ('Helpdesk', 'Helpdesk-Vendor') AND active_status = 'True'");
        const requested_by = await pool.query("SELECT * FROM User_Master WHERE role NOT IN ('Helpdesk', 'Helpdesk-Vendor') AND active_status = 'True'")
        res.json({
