...
app.get('/user-access/:username', async (req, res) => {
    ...
    try {
        const query = 'SELECT * FROM Ticket where requested_by = $1 OR created_by = $1 ORDER BY created_time DESC';
...
app.get('/helpdesk-access/', async (req, res) => {
    const { username } = req.params;
    try {
        const query = 'SELECT * FROM Ticket where assigned_staff = NULL ORDER BY created_time DESC';
        const value = [username];
        const result = await pool.query(query, value);
        res.json(result.rows);
    } catch (error) {
